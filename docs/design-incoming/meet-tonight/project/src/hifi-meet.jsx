...
/* ============================================================ */
/*  Item A · Meet-Ronki — One continuous shot                   */
/*  Hi-fi, playable. 6 beats. Camera never cuts.                */
/* ============================================================ */
...
/* ── Cave background (painterly, layered, animated) ─────── */
function CaveScene({ phase, pickedTint, pickedDeep }) {
  // phase: 'approach' | 'shelf' | 'wobble' | 'hatch' | 'meet' | 'name' | 'close'
  const cameraScale =
    phase === 'approach' ? 1.0 :
    phase === 'shelf'    ? 1.4 :
    phase === 'wobble'   ? 1.7 :
  ...
  // Lantern intensity rises during the shot, peaks at hatch, settles
  const lantern =
    phase === 'approach' ? 0.55 :
    phase === 'shelf'    ? 0.85 :
    phase === 'wobble'   ? 1.0 :
    ...
    <div style={{
      ...
      <div style={{
        ...
        <svg viewBox="0 0 400 700" preserveAspectRatio="xMidYMid slice"
          ...
          <defs>
            <radialGradient id="bgWarm" cx="50%" cy="65%" r="55%">
              <stop offset="0%" stopColor="#3a2310" />
              <stop offset="50%" stopColor="#1c1108" />
              <stop offset="100%" stopColor="#050302" />
            </radialGradient>
            <radialGradient id="lant" cx="50%" cy="58%" r="35%">
            ...
            <radialGradient id="shelfG" cx="50%" cy="50%" r="60%">
              <stop offset="0%" stopColor="#5a3a20" />
              <stop offset="100%" stopColor="#1c1108" />
            </radialGradient>
          </defs>

          <rect width="400" height="700" fill="url(#bgWarm)" />

          {/* Side cave walls — painted strokes */}
          ...
          {/* Texture brush strokes on walls */}
          <g opacity="0.4">
          ...
          {/* Cave floor */}
          <path d="M0 600 Q200 580, 400 600 L400 700 L0 700 Z" fill="url(#floorG)" />

          {/* Stone shelf — appears mid-shot */}
          <g style={{
            opacity: phase === 'approach' ? 0 : 1,
            transition: 'opacity 1400ms ease',
          }}>
            <ellipse cx="200" cy="430" rx="135" ry="14" fill="#0a0604" opacity="0.7"/>
            <path d="M65 420 Q200 405, 335 420 L335 445 Q200 455, 65 445 Z"
                  fill="url(#shelfG)" />
          ...
          <rect width="400" height="700" fill="url(#lant)" />

          {/* Lantern source — top left */}
          <g style={{
        ...
        {[...Array(18)].map((_, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              animation: `mote ${8 + (i % 5)}s ease-in-out infinite ${(i * 0.4) % 3}s`,
            }} />
          );
        })}
      </div>

      {/* Hatch flash — full-screen, only during 'hatch' */}
      {phase === 'hatch' && (
        <div key="flash" style={{
          position: 'absolute', inset: 0,
          background: `radial-gradient(circle at 50% 55%, ${pickedTint || '#fef3c7'}cc 0%, transparent 50%)`,
          animation: 'flash 1400ms ease-out forwards',
          pointerEvents: 'none',
        }} />
      )}
...
/* ── Egg (painterly, with hatch animation) ─────────────── */
  ...
  return (
    <button
      ...
      style={{
        ...
        animation: wobble ? 'eggWobble 1.4s ease-in-out infinite' : 'none',
        filter: picked ? 'drop-shadow(0 0 18px rgba(252,211,77,.7))' : 'drop-shadow(0 6px 10px rgba(0,0,0,.5))',
      }}
    >
      <svg viewBox="0 0 60 80" width={size} height={size * 1.28} style={{ display: 'block' }}>
        <defs>
          <radialGradient id={`eg-${tint}`} cx="35%" cy="32%" r="65%">
        ...
        <ellipse cx="30" cy="42" rx="22" ry="34" fill={`url(#eg-${tint})`}/>
        {/* speckles */}
...
/* ── Ronki chibi (chibi placeholder, painterly hand) ─── */
function Ronki({ size = 130, variant, blink = false, awakening = 0, breathe = false }) {
  ...
  return (
    <div style={{
      ...
      <svg viewBox="0 0 130 130" width={size} height={size}>
        ...
        {/* body — pear */}
        <ellipse cx="65" cy="78" rx="38" ry="42" fill={`url(#bd-${v.id})`}/>
        {/* belly */}
        <ellipse cx="65" cy="88" rx="22" ry="26" fill={`url(#bel-${v.id})`}/>
        {/* horns */}
...
/* ── Voice line bubble (Drachenmutter or Ronki) ────── */
function VoiceLine({ who, text, dark = true, key: k }) {
  return (
    <div style={{
      position: 'absolute', bottom: 90, left: 28, right: 28,
      animation: 'lineIn 800ms ease-out',
      pointerEvents: 'none',
      textAlign: 'center',
    }}>
      <div style={{
      ...
      <p style={{
        margin: 0,
        font: `400 ${who === 'Ronki' ? 17 : 15}px/1.45 "Nunito"`,
        fontStyle: 'italic',
        color: dark ? 'rgba(255,242,217,.92)' : '#3a2410',
        textWrap: 'pretty',
      }}>
        {text}
      </p>
...
/* ── Audio waveform ticker (suggests TTS) ────────── */
...
/* ── ITEM A — Meet-Ronki ────────────────────────── */
function MeetRonki({ onDone }) {
  const [phase, setPhase] = useState('approach');
  const [picked, setPicked] = useState(null);
  const [name, setName] = useState('');
  const [voiceKey, setVoiceKey] = useState(0);

  // Auto-advance approach → shelf
  useEffect(() => {
    if (phase === 'approach') {
    ...
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
      ...
      {(phase === 'shelf' || phase === 'wobble' || phase === 'hatch') && (
        <div style={{
          ...
          gap: 18, padding: '0 36px',
          width: '88%',
          justifyItems: 'center',
        }}>
          {EGG_VARIANTS.map((e, i) => {
            const isPicked = picked === e.id;
            const dim = picked && !isPicked;
            return (
              <Egg
                key={e.id}
                tint={e.tint} deep={e.deep}
                size={62}
                picked={isPicked}
                dim={dim}
      ...
      {/* Voice line */}
      {cur && phase !== 'hatch' && (
        <VoiceLine key={voiceKey} who={cur.who} text={cur.text} />
      )}

      {/* Waveform during voiced phases (~3s window) */}
      {cur && (phase === 'approach' || phase === 'shelf' || phase === 'meet' || phase === 'close') && (
        <Waveform active />
      )}

      {/* Name input — appears under voice line during 'name' */}
      {phase === 'name' && (
        <div style={{
          position: 'absolute', bottom: 38, left: 28, right: 28,
          display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center',
          animation: 'lineIn 700ms ease 400ms backwards',
        }}>
          <input
            type="text"
            value={name}
          ...
          />
          <button
            onClick={() => { if (name.trim()) { setPhase('close'); setVoiceKey(v => v + 1); } }}
            disabled={!name.trim()}
            style={{
              padding: '12px 28px',
              borderRadius: 999,
      ...
      {/* Tap-to-close after closing line plays */}
      {phase === 'close' && (
        <button
          onClick={onDone}
          style={{
            ...
            paddingBottom: 26,
            animation: 'lineIn 800ms ease 3000ms backwards',
          }}>
...
window.MeetRonki = MeetRonki;
window.Ronki = Ronki;
window.EGG_VARIANTS = EGG_VARIANTS;
