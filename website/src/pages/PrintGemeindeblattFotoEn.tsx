/**
 * English-language variant of the Gemeindeblatt photo composition.
 *
 * Identical layout and canvas to `PrintGemeindeblattFoto.tsx` — only the
 * PhoneMockup switches to `locale="en"`, which swaps the on-screen strings
 * (eyebrow, heading, task labels, nav) to English. Everything else
 * (SVG sketch, watermark, Ronki icon, glows) stays language-agnostic.
 *
 * Intended for Marc's international outreach (Reddit, Hacker News, English
 * parent creators) where the German original reads as opaque.
 *
 * Canvas still matches A4 landscape so he can print or screenshot the same
 * way — see header of the DE file for the aspect-ratio notes.
 */

import { useEffect, useState } from 'react';
import { PhoneMockup } from '../components/PhoneMockup';

export default function PrintGemeindeblattFotoEn() {
  ...
  return (
    <div className={`print-root ${exportMode ? 'export-mode' : ''}`}>
      {!exportMode && (
        <div className="no-print" style={instructionBarStyle}>
          <strong style={{ fontWeight: 700 }}>
            Community-article photo · EN · 4320×3054 (A4 landscape @ 3×)
          </strong>
          <span style={{ opacity: 0.7, marginLeft: 10, fontSize: 11.5 }}>
            Export mode hides this bar · screenshot via DevTools
          </span>
      ...
      <div className="photo-canvas">
        ...
        {/* Phone + floating Ronki companion */}
        <div className="stage">
          <div className="phone-wrapper">
            <PhoneMockup variant="morgen-anchor" scale={3} locale="en" />
          </div>
          <img
        ...
        {/* Hand-drawn routine-circle sketch — implies the morning rhythm
            ...
            three, dashed on the fourth = "still open". Identical to the
            DE file because it's purely geometric, no text. */}
        <svg
          className="sketch"
          viewBox="0 0 800 140"
