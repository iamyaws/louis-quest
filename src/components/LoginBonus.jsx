...
export default function LoginBonus({ day, streak, onCollect }) {
  // day = current day index (0-6) in the 7-day cycle
  // streak = total consecutive login days
...
function RewardSlot({ reward, index, currentDay, onCollect }) {
  ...
  return (
    <div
      style={style}
      onClick={isCurrent ? onCollect : undefined}
      role={isCurrent ? "button" : undefined}
      aria-label={isCurrent ? `Tag ${reward.day} Bonus abholen: ${reward.amount} HP` : undefined}
    >
      {/* Day number */}
      <div style={{
        fontFamily: "'Fredoka',sans-serif", fontSize: ".7rem", fontWeight: 700,
        color: isClaimed ? "#92400E" : isCurrent ? "#92400E" : T.textLight,
      ...
      {/* Icon */}
      <div style={{
        fontSize: isJackpot ? "1.5rem" : "1.2rem",
        lineHeight: 1,
        filter: isFuture ? "grayscale(0.8)" : "none",
      }}>
        {isClaimed ? "\u2705" : isFuture ? "\uD83D\uDD12" : reward.icon}
      </div>

      {/* Amount */}
      <div style={{
        fontFamily: "'Fredoka',sans-serif",
        fontSize: isJackpot && isCurrent ? "1rem" : ".85rem",
        fontWeight: 700,
        color: isClaimed ? "#059669" : isCurrent ? "#D97706" : T.textLight,
      }}>
        +{reward.amount}
      </div>

      {/* Collect button for current day */}
      {isCurrent && (
        <div style={{
          fontFamily: "'Fredoka',sans-serif", fontSize: ".7rem", fontWeight: 700,
          color: "white",
      ...
      {isJackpot && !isClaimed && (
        <div style={{
          ...
          fontFamily: "'Fredoka',sans-serif", fontSize: ".55rem", fontWeight: 700,
          color: "white",
