...
export default function Achievements() {
  ...
  function isUnlocked(check) {
    switch (check) {
      case 'lvl5':    return level >= 5;
      case 'lvl10':   return level >= 10;
      case 'tasks50': return state.hist.length >= 50;
      case 'tasks100': return state.hist.length >= 100;
      case 'gear3':   return (state.acc || []).length > 0;
      default:        return false;
    }
  }
  const badges = BADGE_DEFS.map(b => ({ ...b, u: isUnlocked(b.check) }));
  ...
  return (
    <div className="view-enter" style={{ minHeight: "100dvh", padding: "12px 16px 100px", background: "#EFF3FB" }}>
      ...
      <div className="game-card" style={{ padding: 20, display: "flex", alignItems: "center", gap: 16, marginBottom: 20, borderRadius: 22 }}>
        ...
        <div>
          <div style={{ fontFamily: "'Fredoka',sans-serif", fontWeight: 800, fontSize: "1.05rem" }}>{state.hero.name}</div>
...
<div style={{ fontFamily: "'Fredoka',sans-serif", fontWeight: 800, fontSize: ".85rem", color: "#64748B", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 12 }}>Badges</div>
