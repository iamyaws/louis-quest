    ...
    <div className="game-card" style={{ padding: 14, marginBottom: 12 }}>
      <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0, gap: 6, minWidth: 130 }}>
          <div style={{ position: "relative", display: "inline-block", cursor: "pointer" }} onClick={onCompanionTap}>
            {state.eggHatched ? (
              <Companion type={state.companionType || "cat"} variant={state.catVariant} mood={companionMood} size={120} stage={stage} gear={state.equippedGear} />
            ) : (
              <Egg type={state.eggType || "dragon"} size={100} progress={state.eggProgress || 0} />
            )}
            <div style={{ position: "absolute", bottom: -4, right: -16, width: 40, height: 40, borderRadius: "50%", overflow: "hidden", border: "2px solid white", boxShadow: "0 2px 8px rgba(0,0,0,0.15)", background: `radial-gradient(circle at 50% 60%, ${state.hero.color}30 0%, transparent 70%)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <div style={{ transform: "scale(1.6) translateY(18%)" }}>
                <HeroSprite shape={state.hero.shape} color={state.hero.color} eyes={state.hero.eyes} hair={state.hero.hair} size={26} level={level} skinTone={state.hero.skinTone} hairColor={state.hero.hairColor} />
              </div>
            </div>
          </div>
          <div style={{ fontFamily: "'Fredoka',sans-serif", fontSize: "1.15rem", fontWeight: 700, color: T.textPrimary, textAlign: "center", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 130 }}>{state.catName || "Begleiter"}</div>
          <div style={{ background: "linear-gradient(135deg, rgba(251,191,36,0.12), rgba(245,158,11,0.06))", border: "1.5px solid rgba(245,158,11,0.15)", borderRadius: 50, padding: "2px 10px", fontFamily: "'Fredoka',sans-serif", fontSize: ".85rem", fontWeight: 700, color: "#A83E2C", display: "inline-flex", alignItems: "center", gap: 4 }}>{stageInfo?.emoji} {stageInfo?.name}</div>
        </div>
        <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          ...
          <div style={{ background: "#E0F2FE40", borderRadius: 14, padding: "8px 10px", display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <div><div style={{ fontFamily: "'Fredoka',sans-serif", fontSize: "1rem", fontWeight: 700, color: "#0284C7" }}>{todayDaily ? `${todayDaily.tempMin}\u00B0 / ${todayDaily.tempMax}\u00B0` : weather?.current ? `${weather.current.temp}\u00B0` : "\u2022\u2022\u2022"}</div></div>
          </div>
          <div style={{ background: "#EDE9FE40", borderRadius: 14, padding: "8px 10px", display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ fontSize: "1.2rem" }}>{"\uD83D\uDCC5"}</span>
            <span style={{ fontFamily: "'Fredoka',sans-serif", fontSize: ".95rem", fontWeight: 700, color: "#6366F1" }}>{dayKey}. {today.getDate()}. {GERMAN_MONTHS[today.getMonth()]}</span>
          </div>
        </div>
      </div>
    </div>
